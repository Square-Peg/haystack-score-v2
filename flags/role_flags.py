...
irrelevant_role_title_regex = re.compile(
    r'academy|ambassador|agent|advisor|boutique|case team|club|consult|fellow|festival|film|field|freelanc|google for startups|self[-\s]?employ|stage|theatre|ment[ee|or]|organi[s|z]er|partner|part[-\s]?time|program|venture|[executive|exec|personal] assist|(stage|event|production|programme|program|office|account) manag|agent|leadership|coach|counsel|whiskey|whisky|non[-\s]?profit|podcast|\bngo\b|\bphilanthro\b|key opinion|influencer|affiliate',
    re.IGNORECASE,
)
