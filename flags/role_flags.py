...
irrelevant_role_title_regex = re.compile(
    r'academy|ambassador|agent|advisor|boutique|case team|club|consult|fellow|festival|film|field|freelanc|google for startups|self[-\s]?employ|stage|theatre|ment[ee|or]|organi[s|z]er|partner|part[-\s]?time|program|venture|[executive|exec|personal] assist|(stage|event|production|programme|program|office|account) manag|agent|leadership|coach|counsel',
    re.IGNORECASE,
)
irrelevant_role_description_regex = re.compile(
    r'advisor|ambassador|boutique|consult|freelanc|google for startups|self[-\s]?employ|part[-\s]?time|software consult|coach|meet[-\s]?up|our service|\bwine\b|whiskey|whisky|outsourc|network',
    re.IGNORECASE,
)
